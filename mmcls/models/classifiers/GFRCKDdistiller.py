...
class GFRCKDdistiller(BaseModel, metaclass=ABCMeta):
    def __init__(self,
                 teacher_cfg,
                 student_cfg,
                 is_vit = False,
        ...
        super(GFRCKDdistiller, self).__init__(
            init_cfg=init_cfg,
    ...
    def forward(self,
        ...
        feats_t = self.teacher.extract_feat(inputs, stage='backbone')
        class_token_t = self.teacher.head.classtoken(feats_t)
        ...
        if mode == 'tensor':
            ...
            return self.student.head.predict(class_token_t, feats_s, data_samples)
        else:
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        if self.is_vit:
            ...
            #logit_s = self.student.head.avg_logits(x)
        else:
            ...
            with torch.no_grad():
                ...
                if self.use_logit:
                    if self.is_vit:
                        ...
                        #logit_t = self.teacher.head.avg_logits(self.teacher.head.pre_logits(fea_t))
                    else:
