...
class SPEKDDistiller(BaseModel, metaclass=ABCMeta):
    def __init__(self,
                 teacher_cfg,
                 student_cfg,
                 is_vit = False,
        ...
        super(DDVTFDDistiller, self).__init__(
            init_cfg=init_cfg,
    ...
    def forward(self,
        ...
        feats_s = self.student.extract_feat(inputs, stage='backbone')

        if mode == 'tensor':
            return self.student.head(feats_s) if self.student.with_head else feats_s
        elif mode == 'loss':
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        if not self.sd:
            ...
            if 'loss_ddvtfd' in all_keys:
                loss_name = 'loss_ddvtfd'
                s_loss[loss_name] = self.distill_losses[loss_name](fea_s[-1][0], fea_t[-1][0])

            if 'loss_spekd' in all_keys:
                loss_name = 'loss_spekd'
                s_loss[loss_name] = self.distill_losses[loss_name](fea_s[-1][0], fea_t[-1][0])

            if 'loss_ddvt' in all_keys:
                loss_name = 'loss_ddvt'
                s_loss[loss_name] = self.distill_losses[loss_name](fea_s[-1][0], logit_t)

            if ('loss_ddkd' in all_keys) and self.use_logit:
                loss_name = 'loss_ddkd'
                s_loss[loss_name] = self.distill_losses[loss_name](logit_s, logit_t)
                s_loss['ori_loss'] = 0.5 * s_loss['ori_loss']
            ...
            if 'loss_gfrckd' in all_keys:
                loss_name = 'loss_gfrckd'
                s_loss[loss_name] = self.distill_losses[loss_name](fea_s[-1][0], fea_t[-1][0])

            if 'loss_msekd' in all_keys:
                loss_name = 'loss_msekd'
                s_loss[loss_name] = self.distill_losses[loss_name](fea_s[-1][0], fea_t[-1][0])
