...
class DDVTLoss(nn.Module):
    ...
    def __init__(self,
        ...
        super(DDVTLoss, self).__init__()
        self.temp = temp
        self.alpha = alpha
        self.beta = beta

        self.align21 = nn.Linear(student_dims, num_classes, bias=True)
        self.align22 = nn.Linear(teacher_dims, num_classes, bias=True)

    def forward(self, fea_s, logit_t):
        cls_s = fea_s[2]
        logit_cls = self.align21(cls_s)

        pred_student = F.softmax(logit_cls / self.temp, dim=1) + 1e-9
        ...
        mc_loss = F.kl_div(log_student_main, teacher_main, reduction='batchmean') * (self.temp**2)

        # 掩盖主类，仅保留非主类
        mask = torch.ones_like(pred_teacher).scatter_(1, max_idx_t, 0).bool()

        # 计算非主类的概率分布
        student_non_main = pred_student[mask].view(logit_cls.size(0), -1)
        teacher_non_main = pred_teacher[mask].view(logit_cls.size(0), -1)
